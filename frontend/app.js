...
function generateBotResponse(input) {
    ...
    if (text.includes('urgencia') || text.includes('emergencia') || text.includes('critico') || text.includes('grave')) {
        return "⚠️ **Protocolos de Urgencia:** Los casos clínicos de mayor urgencia respiratoria incluyen: Neumotórax a tensión, Tromboembolismo Pulmonar (TEP) masivo, y Síndrome de Distrés Respiratorio Agudo (SDRA). Se requiere intervención inmediata.";
    } 
    // Neumonía
    else if (text.includes('neumonía') || text.includes('neumonia')) {
        return "Basado en protocolos estándar: Para neumonía extrahospitalaria típica, se sugiere iniciar con Amoxicilina o un Macrólido. ¿Te gustaría consultar una guía clínica específica?";
    }
    // Tratamientos generales
    else if (text.includes('tratamiento') || text.includes('receta')) {
        return "El tratamiento dependerá del diagnóstico final. Para infecciones bacterianas se usan antibióticos; para derrames pleurales, drenaje o diuréticos. ¿Tienes algún diagnóstico presuntivo en mente?";
    }
    // Saludos básicos (solo si es muy corto o es el único contexto)
    else if ((text.includes('hola') || text.includes('buenos dias')) && text.length < 15) {
        return "¡Hola, Doctor! ¿En qué caso clínico te puedo asistir hoy?";
    } 
    // Respuesta por defecto
    else {
        return "Interesante pregunta. Como asistente IA, analizo las palabras clave. ¿Podrías ser más específico con el síntoma o la anomalía pulmonar que deseas consultar?";
    }
}
